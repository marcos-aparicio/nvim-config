...
function M.setup_buffer_keymaps()
  ...
  vim.keymap.set(
    ...
    "<localleader>c",
    function()
      task_mgmt.toggle_cancelled_state()
    end,
    { buffer = true, desc = "Toggle cancelled state (moves to Cancelled tasks section)" }
  )

  -- Diary note keymaps
  vim.keymap.set("n", "<leader>dh", function()
    local today = os.date("%Y-%m-%d", os.time())
    diary.open_diary_note_for_date(today)
  end, { buffer = true, desc = "Open today's diary note" })

  vim.keymap.set("n", "<leader>da", function()
    local yesterday = os.date("%Y-%m-%d", os.time() - 24 * 60 * 60)
    diary.open_diary_note_for_date(yesterday)
  end, { buffer = true, desc = "Open yesterday's diary note" })

  vim.keymap.set("n", "<leader>dm", function()
    local tomorrow = os.date("%Y-%m-%d", os.time() + 24 * 60 * 60)
    diary.open_diary_note_for_date(tomorrow)
  ...
  vim.keymap.set("n", "<leader>dc", function()
    vim.ui.input({ prompt = "Enter date (YYYY-MM-DD): " }, function(input)
      if input and input:match("^%d%d%d%d%-%d%d%-%d%d$") then
        diary.open_diary_note_for_date(input)
      else
  ...
  vim.keymap.set("n", "<leader>msh", function()
    local today = os.date("%Y-%m-%d", os.time())
    diary.open_weekly_note_for_date(today)
  ...
  vim.keymap.set("n", "<leader>msa", function()
    ...
    diary.open_weekly_note_for_date(last_week)
