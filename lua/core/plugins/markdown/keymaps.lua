...
function M.setup_buffer_keymaps()
  ...
  -- Tag search keymaps
  vim.keymap.set("n", "<leader>kt", telescope.pick_tag_and_search,
    { buffer = true, desc = "Pick tag and search" })
  vim.keymap.set("n", "<leader>ka", telescope.pick_task_tag_and_search,
    { buffer = true, desc = "Pick task tag and search" })

  -- Text formatting keymaps
  vim.keymap.set("n", "<leader>md", toggle_bullet_point,
    { buffer = true, desc = "Toggle bullet point (dash)" })
  vim.keymap.set("n", "<leader>ml", toggle_todo_checkbox,
  ...
  -- Link handling
  vim.keymap.set("n", "<leader>mo", open_markdown_link,
    { buffer = true, desc = "Open markdown link in browser" })

  -- Task management
  vim.keymap.set("n", "<localleader>d", task_mgmt.toggle_task_state,
    { buffer = true, desc = "Toggle task state (blank → progress → done → blank)" })

  -- Task search keymaps
  vim.keymap.set("n", "<leader>tc", telescope.search_completed_tasks,
    { buffer = true, desc = "Search for completed tasks" })
  vim.keymap.set("n", "<leader>ti", telescope.search_progress_tasks,
    { buffer = true, desc = "Search for tasks in progress" })
  vim.keymap.set("n", "<leader>tt", telescope.search_incomplete_tasks,
