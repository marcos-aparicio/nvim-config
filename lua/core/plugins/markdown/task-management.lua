...
function M.toggle_task_state(direction)
  ...
  local label_cancelled = "#_cancelled"
  local tasks_heading = "## Completed tasks"
  local cancelled_heading = "## Cancelled tasks"
       ...
       blank = {
         ...
         sub_transforms = function()
           for i = 1, #chunk do
             ...
             chunk[i] = chunk[i]:gsub("%s+#_cancelled%s+%d%d%d%d%d%d%-%d%d%d%d", "")
...
function M.toggle_cancelled_state()
  local label_cancelled = "#_cancelled"
  local cancelled_heading = "## Cancelled tasks"
  local timestamp = os.date("%y%m%d-%H%M")
  ...
  local is_cancelled = chunk[1]:match("#_cancelled")

  if is_cancelled then
    -- Remove cancelled state and move back
    chunk[1] = chunk[1]:gsub("%s+#_cancelled%s+%d%d%d%d%d%d%-%d%d%d%d", "")
    chunk[1] = chunk[1]:gsub("%s+`untoggled`", "")
    chunk[1] = chunk[1]:gsub("^(%s*%- )%[c%]", "%1[ ]")
    vim.api.nvim_buf_set_lines(buf, chunk_start, chunk_end + 1, false, chunk)
    vim.notify("Cancelled state removed", vim.log.levels.INFO)
  else
    -- Mark as cancelled and move to cancelled section
    for i = chunk_end, chunk_start, -1 do
      table.remove(lines, i + 1)
    end

    -- Add cancelled label to the first line and change checkbox to [c]
    chunk[1] = chunk[1]:gsub("%s+#_progress", "")
    chunk[1] = chunk[1]:gsub("%s+#_done%s+%d%d%d%d%d%d%-%d%d%d%d", "")
    chunk[1] = chunk[1]:gsub("^(%s*%- )%[[x %-]%]", "%1[c]")
    chunk[1] = chunk[1]:gsub("^(%s*%- %[c%]%s*)", "%1" .. label_cancelled .. " " .. timestamp .. " ")

    local heading_index = find_heading_by_treesitter(cancelled_heading)
    if heading_index then
      heading_index = heading_index + 1
      for _, cLine in ipairs(chunk) do
        table.insert(lines, heading_index + 1, cLine)
      ...
      table.insert(lines, cancelled_heading)
      for _, cLine in ipairs(chunk) do
        table.insert(lines, cLine)
      end
    end
    ...
    vim.api.nvim_buf_set_lines(buf, 0, -1, false, lines)
    vim.notify("Marked as Cancelled", vim.log.levels.INFO)
  end
