-- Switch any kulala @variable that is declared on multiple lines (all but one
-- commented out) via a picker. Options are read straight from the current
-- buffer, so the .http file stays the single source of truth — this module only
-- automates the comment/uncomment. Renders through Telescope when
-- telescope-ui-select / dressing.nvim is installed (it uses vim.ui.select).
local function kulala_toggle()
  local buf = 0
  local lines = vim.api.nvim_buf_get_lines(buf, 0, -1, false)
  local pat = "^%s*(#?)%s*@([%w_%-]+)%s*=%s*(.-)%s*$"
  local groups, order = {}, {}

  for i, line in ipairs(lines) do
    local hash, name, val = line:match(pat)
    if name then
      ...
      groups[name][#groups[name] + 1] =
        { lnum = i - 1, active = hash == "", content = ("@%s = %s"):format(name, val) }
    end
  end

  local switchable = vim.tbl_filter(function(n) return #groups[n] > 1 end, order)
  if #switchable == 0 then
    vim.notify("Kulala: no switchable @var groups in this file", vim.log.levels.WARN)
    return
  end

  local function choose(name)
    local entries, items = groups[name], {}
    for _, e in ipairs(entries) do
      items[#items + 1] = (e.active and "● " or "○ ") .. e.content
    end
    vim.ui.select(items, { prompt = "Set @" .. name }, function(_, idx)
      if not idx then return end
      for j, e in ipairs(entries) do
        vim.api.nvim_buf_set_lines(buf, e.lnum, e.lnum + 1, false,
          { j == idx and e.content or "# " .. e.content })
      end
      vim.notify("kulala: " .. entries[idx].content)
    end)
  end

  local cur, under = vim.api.nvim_win_get_cursor(0)[1] - 1, nil
  for _, name in ipairs(switchable) do
    for _, e in ipairs(groups[name]) do
      if e.lnum == cur then under = name end
    end
  end

  if under then
    choose(under)
  elseif #switchable == 1 then
    choose(switchable[1])
  else
    vim.ui.select(switchable, { prompt = "Which variable?" }, function(n)
      if n then choose(n) end
    end)
  end
end
...
return
{
  ...
  config = function(_, opts)
    ...
    -- Switch a kulala @variable group (e.g. store) via a picker.
    vim.api.nvim_create_user_command("KulalaToggle", kulala_toggle, {})

    local function map(buf)
      vim.keymap.set("n", "<leader>pv", kulala_toggle,
        { buffer = buf, desc = "Kulala: switch a @variable" })
    end
    vim.api.nvim_create_autocmd("FileType", {
      pattern = { "http", "rest" },
      callback = function(a) map(a.buf) end,
    })
    -- also bind the buffer that triggered plugin load (its FileType already fired)
    for _, b in ipairs(vim.api.nvim_list_bufs()) do
      local ok, ft = pcall(vim.api.nvim_get_option_value, "filetype", { buf = b })
      if ok and (ft == "http" or ft == "rest") then map(b) end
    end
  end,
}
